...
from lib.realformer import RealFormerEncoder
from lib.senet import se_resnet18
...
class ModifiedResnet(nn.Module):
    ...
    def __init__(self, usegpu=True):
        ...
        self.model = psp_models['se_resnet18'.lower()]()
...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, modal_feats: Tensor) -> Any:
        ...
        if self.separate is False:
            D = modal_feats.shape[1] - 1
            temp_emb = []

            t = modal_feats.shape[1] - 1
            ...
            temp_emb = torch.cat(temp_emb, dim=0).unsqueeze(dim=0).to(self.device)
            temp_emb = self.embedding(temp_emb)
            ...
            for modal in modal_feats:
                ...
                temp_emb.append(self.embedding(indices.unsqueeze(dim=0)))
            return temp_emb
